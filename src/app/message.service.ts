...
export class MessageService {
  ...
  getuser() {
    let map: string[] = this.messages.map((mes) => mes.to);
    let s = new Set(map);
    return Array.from(s);
  }
  getFolder() {
    let map = this.messages.map((mes) => mes.folder);
    let s = new Set(map);
  ...
  getMessageByUser() {
    return this.messages.filter((message) => message.to === this.user);
  }
  getMessageByFolder(folder: string) {
    return this.messages.filter((m) => m.folder === folder && m.to===this.user);
  }
  getMessageDetail(){
